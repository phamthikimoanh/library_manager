...
class BookCardsController < ApplicationController
  ...
  def check_status
    if @book_card.status == true && count_book_card == true
      ...
      book_in_stock = @book_card.book.books_total + 1
      @book_card.book.update_columns(books_total: book_in_stock, status: 1)
    else
      @book_card.update_column(:status , 0)
      book_in_stock = @book_card.book.books_total - 1
      @book_card.book.update_columns(books_total: book_in_stock, status: 0)
    end
  end
